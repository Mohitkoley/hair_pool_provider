...
class BookingDetailsController extends GetxController implements GetxService {
  ...
  void onInit() {
    ...
    if (Get.find<SplashController>()
            .configModel
            .content
            ?.providerCanCancelBooking ==
        0) {
      statusTypeList.remove('canceled');
    }
  }

  Future<void> getBookingDetails(String bookingID,
      {bool reload = true, bool initEditBooking = true}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (initEditBooking) {
        Get.find<BookingEditController>().getServiceListBasedOnSubcategory(
            subCategoryId: bookingDetails?.content?.subcategoryId ?? "");
        Get.find<BookingEditController>()
            .initializedControllerValue(_bookingDetails?.content);
      }
  ...
  Future<void> getBookingSubDetails(String bookingID,
      {bool reload = true}) async {
    Response response =
        await bookingDetailsRepo.getSubBookingDetails(bookingID);

    if (response.statusCode == 200) {
      _subBookingDetails = BookingDetailsModel.fromJson(response.body);
      Get.find<BookingEditController>()
          .initializedControllerValue(_subBookingDetails?.content);
      subBookingDropDownValue =
          _subBookingDetails?.content?.bookingStatus ?? "";
    } else {
      ApiChecker.checkApi(response);
    }
    update();
  ...
  Future<void> acceptBookingRequest(String bookingId) async {
    ...
    update();
    Response response =
        await bookingDetailsRepo.acceptBookingRequest(bookingId);
    if (response.statusCode == 200 &&
      ...
      Get.find<BookingRequestController>().getBookingRequestList(
          Get.find<BookingRequestController>().bookingStatus, 1);
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> ignoreBookingRequest(String bookingId) async {
    ...
    update();
    Response response =
        await bookingDetailsRepo.ignoreBookingRequest(bookingId);
    if (response.statusCode == 200) {
      showCustomSnackBar(response.body["message"],
          type: ToasterMessageType.success);
      Get.find<BookingRequestController>().getBookingRequestList(
  ...
  Future<void> cancelSubBooking(
      {required String bookingId, required String subBookingId}) async {
    _isIgnoreButtonLoading = true;
    update();
  ...
  Future<void> changeBookingStatus(String bookingId,
    ...
    if (bookingStatus != null &&
      ...
      if (response.statusCode == 200 &&
        ...
        showCustomSnackBar(response.body['message'].toString().capitalizeFirst,
          ...
          response.body["response_code"] == "default_403") {
  ...
  Future<void> changeBookingStatusNew(
    String bookingId, {
    String? bookingStatus,
  }) async {
    _isStatusUpdateLoading = true;
    ...
    // }
    Response response = await bookingDetailsRepo.changeBookingStatusNew(
      bookingId,
      bookingStatus ?? "",
      multiParts,
    );
  ...
  Future<bool> sendBookingOTPNotification(String? bookingId,
      {bool shouldUpdate = true, bool resend = false}) async {
    if (shouldUpdate) {
      _hideResendButton = true;
      update();
    }
    Response response =
        await bookingDetailsRepo.sendBookingOTPNotification(bookingId);
    bool isSuccess;
    if (response.statusCode == 200) {
      isSuccess = true;
    } else {
  ...
  void updateServicePageCurrentState(
      BookingDetailsTabControllerState bookingDetailsTabControllerState,
  ...
  bool isShowChattingButton(
      BookingDetailsContent? bookingDetails, TabController? tabController) {
    return ((bookingDetails != null) &&
        ...
        ((bookingDetails.serviceman != null ||
            ...
            (bookingDetails.customer != null ||
                bookingDetails.subBooking?.customer != null)));
  }
