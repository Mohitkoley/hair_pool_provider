...
class DashboardController extends GetxController
  ...
  void changeTypeOfShowBookingStatus(
      {required bool status, bool shouldUpdate = true}) {
      ...
      for (var element in servicemanList) {
        {
          if (element['user']['is_active'] == 1) {
            dashboardServicemanList
                .add(DashboardServicemanModel.fromJson(element));
          }
        }
      }
  ...
  void removeSubscriptionItem(String id, {bool shouldUpdate = true}) {
    dashboardSubscriptionList
        .removeWhere((element) => element.subCategoryId == id);
    if (shouldUpdate) {
      update();
    }
  }
  ...
  Future<void> getMonthlyBookingsDataForChart(String year, String month) async {
    _monthlyStatsList = [];
    Response response =
        await dashBoardRepo.getMonthlyDashBoardChartData(year, month);
    if (response.statusCode == 200) {
      _monthlyStatsList = [];
      _mStatsList = [];
      _monthlyChartList = [];
      ...
      for (int i = 0; i < _monthlyStatsList.length; i++) {
        _mStatsList[_monthlyStatsList[i].day!] =
            double.tryParse(_monthlyStatsList[i].sums!)!.toDouble();
      }
  ...
  Future<void> getYearlyBookingsDataForChart(String year) async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < _yearlyStatsList.length; i++) {
        _yStatsList[monthMap[_yearlyStatsList[i].month]!] =
            double.tryParse(_yearlyStatsList[i].sums!)!.toDouble();
      }
